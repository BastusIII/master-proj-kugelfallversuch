...
void get_boardid_i2c(BEAGLEBONE_ID *boneid)
{
	...
	if (dev != NULL)
	{
		...
		if (!r)
		{
			...
			/* 20140121 Max Fischer: detect Beagle Bone Black */
			else if (strcmp(bd.bdname, "A335BNLT")==0) boneid->basebd_type = bb_BeagleBoneBlack;
	...
	{
		kprintf("BeagleBoneBlack detected\n");
	}
	else
	{
 ...
 * set the voltage to 1.35V for 1GHz on BBB
 * (raw copy from detect_frequency_using_power() just to make it work,
 *  detectid.c might need some cleanup...)
 */
unsigned long set_1GHz_power()
{
	omap_dev_t	*dev;
	uint32_t	stop = 0;
	uint8_t		val[40]= { 0, 0 };
	...
	{
		dev->slave_addr = 0x24;
	    /* power setting sequence, using u-boot
	    i2c mw 24 0b 72 1
	    i2c mw 24 0f 11 1
	    i2c mw 24 0b 72 1
	    i2c mw 24 0f 11 1
	...
	*/
		/* Write request to Power Chip */
		stop = 0;
		val[0] = 0x0b; val[1] = 0x72;
		rw   = omap_send(dev, val, 2, stop);
		val[0] = 0x0f; val[1] = 0x11;
		rw   = omap_send(dev, val, 2, stop);
		val[0] = 0x0b; val[1] = 0x72;
		rw   = omap_send(dev, val, 2, stop);
		val[0] = 0x0f; val[1] = 0x11;
		rw   = omap_send(dev, val, 2, stop);
		val[0] = 0x0b; val[1] = 0x6c;
		rw   = omap_send(dev, val, 2, stop);
		val[0] = 0x11; val[1] = 0x86;
		rw   = omap_send(dev, val, 2, stop);
		val[0] = 0x0b; val[1] = 0x6c;
		rw   = omap_send(dev, val, 2, stop);
		val[0] = 0x11; val[1] = 0x86;
