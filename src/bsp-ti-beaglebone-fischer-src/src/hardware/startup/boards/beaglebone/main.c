...
extern unsigned long set_1GHz_power();
...
/* 20140121 Max Fischer
	copied from Peripheral_PLL_Config, not verified
	against the spec, just to make MPU clock setting work.
	Especially M2 semantics is quite adhoc...
*/
void MPU_PLL_Config(uint32_t N, uint32_t M, uint32_t M2)
{
	uint32_t clkmode, clksel, div_m2;

	clkmode = in32(AM335X_CM_CLKMODE_DPLL_MPU);
	clksel = in32(AM335X_CM_CLKSEL_DPLL_MPU);
	div_m2 = in32(AM335X_CM_DIV_M2_DPLL_MPU);

	/* Set the PLL to bypass Mode */
	out32(AM335X_CM_CLKMODE_DPLL_MPU, DPLL_MN_BYP_MODE);

	while(in32(AM335X_CM_IDLEST_DPLL_MPU) != ST_MN_BYPASS);

	clksel = clksel & (~DPLL_MULT_MASK);
	clksel = clksel | (DPLL_MULT(M) | DPLL_DIV(N));
	out32(AM335X_CM_CLKSEL_DPLL_MPU, clksel);

	div_m2 = div_m2 & ~DPLL_CLKOUT_DIV_MASK;
	div_m2 = div_m2 | M2;
	out32(AM335X_CM_DIV_M2_DPLL_MPU, div_m2);

	clkmode = clkmode | DPLL_LOCK_MODE;
	out32(AM335X_CM_CLKMODE_DPLL_MPU, clkmode);

	while(in32(AM335X_CM_IDLEST_DPLL_MPU) != 0x1);
...
main(int argc, char **argv, char **envv)
{
	...
	if (cpu_freq == 0)
	{
		...
		/* 20140121 Max Fischer
		 * if we have no cpu frequency preset and we are on a BBB, we use 1GHz
		 */
		if ( boneid.basebd_type == bb_BeagleBoneBlack ){
			cpu_freq = BEAGLEBONE_MPUFREQ_1GHZ;
		}
	}

	/* 20140121 Max Fischer
	 * in case we use 1GHz, first the voltage has to be set to 1.35V
	 * then the clocks are set.
	 */
	if ( cpu_freq == BEAGLEBONE_MPUFREQ_1GHZ ){
		set_1GHz_power();
		CORE_PLL_Config(24, 23, 1000, 10, 8, 4);
		MPU_PLL_Config(23, 1000, 1);
	}
